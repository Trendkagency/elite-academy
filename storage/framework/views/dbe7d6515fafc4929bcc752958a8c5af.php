        ...
        if (!window.isStepAnswered(window.currentStep)) {
            if (window.Toast) {
                window.Toast.warning(
                    "<?php echo e(app()->getLocale() === 'ar' ? '⚠️ يرجى اختيار إجابة للسؤال الحالي أولاً قبل الانتقال للسؤال التالي.' : '⚠️ Please select an answer for the current question before advancing.'); ?>",
    ...
    if (targetStepIdx > window.currentStep) {
        for (let i = 0; i < targetStepIdx; i++) {
            if (!window.isStepAnswered(i)) {
                if (window.Toast) {
                    window.Toast.warning(
                        `<?php echo e(app()->getLocale() === 'ar' ? '⚠️ يرجى إجابة السؤال رقم (' : '⚠️ Please answer question #'); ?>${i + 1}<?php echo e(app()->getLocale() === 'ar' ? ') أولاً قبل الانتقال لأسئلة لاحقة.' : ' first before skipping ahead.'); ?>`,
                        "<?php echo e(app()->getLocale() === 'ar' ? 'إجابة السؤال مطلوبة' : 'Answer Required'); ?>"
                    );
                }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleQuizTimeExpired() {
        ...
        if (window.Toast) {
            window.Toast.error(
                "<?php echo e(app()->getLocale() === 'ar' ? '⚠️ انتهى الوقت المحدد للواجب! يتم الآن إرسال إجاباتك وتقييمها تلقائياً...' : '⚠️ Time is up! Submitting and evaluating your answers automatically...'); ?>",
