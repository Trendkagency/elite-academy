    ...
    <div class="w-full max-w-md bg-white rounded-3xl p-8 sm:p-10 border border-slate-200/90 shadow-xl space-y-6">
        <div class="text-center space-y-2">
            ...
            <p class="text-xs text-slate-500 font-mono">
                <?php echo e(app()->getLocale() === 'ar' ? 'انضم إلى أكاديمية إيليت وابدأ رحلة التعلّم التفاعلي.' : 'Join Elite Academy and start interactive learning.'); ?>
        ...
        <form id="registerForm" action="<?php echo e(route('ajax.register')); ?>" method="POST" class="space-y-4" novalidate>
            <?php echo csrf_field(); ?>
            ...
            <div class="space-y-1.5" id="group-name">
                <div class="flex justify-between items-center">
                    <label for="reg-name" class="text-xs font-bold text-slate-700">
                        <?php echo e(app()->getLocale() === 'ar' ? 'الاسم بالكامل' : 'Full Name'); ?>
                ...
                <input 
                    ...
                    required 
                    autocomplete="name"
                    placeholder="<?php echo e(app()->getLocale() === 'ar' ? 'مثال: أحمد محمد علي' : 'e.g. David Kovacs'); ?>" 
            ...
            <div class="space-y-3 pt-1">
                <label class="text-xs font-bold text-slate-700 flex items-center justify-between">
                    ...
                    <span class="text-[10px] font-semibold text-teal-600 bg-teal-50 px-2.5 py-0.5 rounded-full border border-teal-100/80">
                        <?php echo e(app()->getLocale() === 'ar' ? 'اختر دورك' : 'Select Role'); ?>
            ...
            <div id="studentFieldsGroup" class="space-y-4 pt-1">
                ...
                <div class="space-y-1.5" id="group-grade">
                    ...
                    >
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $gradeLevels ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $g): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                            <option value="<?php echo e($g->id); ?>"><?php echo e(__($g->name)); ?></option>
                ...
                <div class="space-y-1.5">
                    <label for="reg-school" class="text-xs font-bold text-slate-700">
                        <?php echo e(app()->getLocale() === 'ar' ? 'اسم المدرسة (اختياري)' : 'School Name (Optional)'); ?>
                    ...
                    <input 
                        ...
                        id="reg-school"
                        name="school_name" 
                        placeholder="<?php echo e(app()->getLocale() === 'ar' ? 'مثال: مدرسة المتفوقين للغات' : 'e.g. Cairo International School'); ?>" 
                        class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-medium text-slate-800 placeholder-slate-400 focus:bg-white focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all outline-none"
                    >
                </div>
            </div>
            ...
            <div class="space-y-1.5" id="group-password">
                <div class="flex justify-between items-center">
                    <label for="reg-password" class="text-xs font-bold text-slate-700">
                        <?php echo e(app()->getLocale() === 'ar' ? 'كلمة المرور' : 'Password'); ?>
            ...
            <button type="submit" id="submitBtn" class="w-full btn-mobile-lg btn-lift text-white bg-teal-600 hover:bg-teal-700 shadow-md shadow-teal-600/20 touch-press mt-2 flex items-center justify-center gap-2 font-bold py-3.5 rounded-2xl transition-all">
                <span id="btnText"><?php echo e(app()->getLocale() === 'ar' ? 'إنشاء الحساب والبدء' : 'Create Account & Start'); ?></span>
                <i class="fa-solid <?php echo e(app()->getLocale() === 'ar' ? 'fa-arrow-left' : 'fa-arrow-right'); ?> text-xs" id="btnIcon"></i>
        ...
        <div class="pt-6 border-t border-slate-100 text-center">
            <p class="text-xs text-slate-500 font-mono">
                ...
                <a href="<?php echo e(route('login')); ?>" class="font-bold text-teal-600 hover:text-teal-700 hover:underline ms-1 inline-flex items-center gap-1">
                    <span><?php echo e(app()->getLocale() === 'ar' ? 'تسجيل الدخول للمنصة' : 'Log In to Portal'); ?></span>
                    <i class="fa-solid <?php echo e(app()->getLocale() === 'ar' ? 'fa-arrow-left' : 'fa-arrow-right'); ?> text-[10px]"></i>
                </a>
            </p>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const isAr = <?php echo e(app()->getLocale() === 'ar' ? 'true' : 'false'); ?>;
    ...
    const checkEmailUrl = "<?php echo e(route('ajax.validate.email-available')); ?>";
    const checkPhoneUrl = "<?php echo e(route('ajax.validate.phone-available')); ?>";
    ...
    if (nameInput) {
        nameInput.addEventListener('input', function () {
            ...
            if (!val) {
                ...
                setFieldState(nameInput, nameFeedback, nameBadge, 'error', isAr ? '<?php echo e(__('app.auth.name_required')); ?>' : 'Name must be at least 3 characters', isAr ? 'قصير جداً' : 'Too short');
            } else {
    ...
    async function checkEmailRealtime(value) {
        ...
        if (!isValidEmailSyntax(email)) {
            setFieldState(emailInput, emailFeedback, emailBadge, 'error', isAr ? '<?php echo e(__('app.auth.invalid_email_format')); ?>' : 'Please enter a valid email address', isAr ? 'غير صحيح' : 'Invalid');
            return false;
        }
        ...
        try {
            ...
            if (data.available) {
                setFieldState(emailInput, emailFeedback, emailBadge, 'success', data.message || (isAr ? '<?php echo e(__('app.auth.email_available')); ?>' : 'Email is available'), isAr ? 'متاح' : 'Available');
                return true;
            } else {
                setFieldState(emailInput, emailFeedback, emailBadge, 'error', data.message || (isAr ? '<?php echo e(__('app.auth.email_already_registered')); ?>' : 'This email is already registered'), isAr ? 'مسجل مسبقاً' : 'Taken');
                return false;
            }
        } catch (err) {
            if (emailSpinner) emailSpinner.classList.add('hidden');
    ...
    async function checkPhoneRealtime(value) {
        ...
        if (phone.length < 7) {
            setFieldState(phoneInput, phoneFeedback, phoneBadge, 'error', isAr ? '<?php echo e(__('app.auth.invalid_phone_format')); ?>' : 'Please enter a valid phone number (at least 7 digits)', isAr ? 'غير صالح' : 'Invalid');
            return false;
        }
        ...
        try {
            ...
            if (data.available) {
                setFieldState(phoneInput, phoneFeedback, phoneBadge, 'success', data.message || (isAr ? '<?php echo e(__('app.auth.phone_available')); ?>' : 'Phone is available'), isAr ? 'متاح' : 'Available');
                return true;
            } else {
                setFieldState(phoneInput, phoneFeedback, phoneBadge, 'error', data.message || (isAr ? '<?php echo e(__('app.auth.phone_already_registered')); ?>' : 'This phone number is already registered'), isAr ? 'مسجل مسبقاً' : 'Taken');
                return false;
            }
        } catch (err) {
            if (phoneSpinner) phoneSpinner.classList.add('hidden');
    ...
    if (passwordInput) {
        passwordInput.addEventListener('input', function () {
            ...
            if (val.length < 8) {
                setFieldState(passwordInput, passwordFeedback, null, 'error', isAr ? '<?php echo e(__('app.auth.password_min_length')); ?>' : 'Password must be at least 8 characters');
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            if (!name || name.length < 3) {
                setFieldState(nameInput, nameFeedback, nameBadge, 'error', isAr ? '<?php echo e(__('app.auth.name_required')); ?>' : 'Please enter your full name (minimum 3 characters)');
                notify(isAr ? '<?php echo e(__('app.auth.name_required')); ?>' : 'Please enter your full name', true);
                nameInput.focus();
                return;
            }

            if (!email || !isValidEmailSyntax(email)) {
                setFieldState(emailInput, emailFeedback, emailBadge, 'error', isAr ? '<?php echo e(__('app.auth.invalid_email_format')); ?>' : 'Please enter a valid email address');
                notify(isAr ? '<?php echo e(__('app.auth.invalid_email_format')); ?>' : 'Please enter a valid email address', true);
                emailInput.focus();
                return;
            }

            if (!phone || phone.length < 7) {
                setFieldState(phoneInput, phoneFeedback, phoneBadge, 'error', isAr ? '<?php echo e(__('app.auth.invalid_phone_format')); ?>' : 'Please enter a valid phone number');
                notify(isAr ? '<?php echo e(__('app.auth.invalid_phone_format')); ?>' : 'Please enter a valid phone number', true);
                phoneInput.focus();
                return;
            }

            if (userType === 'student' && !gradeLevel) {
                setFieldState(gradeSelect, gradeFeedback, null, 'error', isAr ? '<?php echo e(__('app.auth.grade_level_required')); ?>' : 'Please select a grade level');
                notify(isAr ? '<?php echo e(__('app.auth.grade_level_required')); ?>' : 'Please select a grade level for student registration', true);
            ...
            if (!password || password.length < 8) {
                setFieldState(passwordInput, passwordFeedback, null, 'error', isAr ? '<?php echo e(__('app.auth.password_min_length')); ?>' : 'Password must be at least 8 characters');
                notify(isAr ? '<?php echo e(__('app.auth.password_min_length')); ?>' : 'Password must be at least 8 characters', true);
                passwordInput.focus();
                return;
            }
            ...
            try {
                ...
                const successMsg = data.message || (isAr ? '<?php echo e(__('app.auth.registered_success')); ?>' : 'Account created successfully! Redirecting...');
