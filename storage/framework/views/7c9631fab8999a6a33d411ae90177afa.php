        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div class="flex items-center gap-5">
                ...
                <div class="space-y-1">
                    ...
                    <h1 class="font-heading text-2xl sm:text-4xl font-black text-white tracking-tight">
                        <?php echo e(__('app.portal.welcome_back')); ?>، <span class="bg-gradient-to-r from-teal-300 to-emerald-400 bg-clip-text text-transparent underline decoration-orange-500 decoration-2 underline-offset-8"><?php echo e(auth()->user()->name ?? __('Learner')); ?>!</span>
                    </h1>
                    <p class="text-slate-300 text-xs sm:text-sm font-mono flex flex-wrap items-center gap-2 pt-1">
                        <span><i class="fa-solid fa-graduation-cap"></i> <?php echo e(__('app.portal.grade_level')); ?>: <strong class="text-teal-300"><?php echo e($studentProfile?->gradeLevel?->name ?: __('Grade 12 STEM')); ?></strong></span>
                        <span class="text-slate-600">•</span>
                        <span><i class="fa-solid fa-school"></i> <?php echo e(__('app.portal.school')); ?>: <strong class="text-slate-200"><?php echo e($studentProfile?->school_name ?: 'Elite STEM Academy Cairo'); ?></strong></span>
                    </p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                <button onclick="document.getElementById('excuseModal').classList.remove('hidden')" class="btn-lift px-5 py-3 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-slate-950 text-xs font-extrabold rounded-2xl shadow-lg shadow-orange-500/20 cursor-pointer flex items-center gap-2 transition-all">
                    <span><i class="fa-solid fa-file-lines"></i></span> <?php echo e(__('app.portal.submit_excuse')); ?>
                ...
                <button onclick="document.getElementById('homeworkExceptionModal').classList.remove('hidden')" class="btn-lift px-5 py-3 bg-teal-600 hover:bg-teal-500 text-white text-xs font-bold rounded-2xl shadow-lg shadow-teal-600/20 cursor-pointer flex items-center gap-2 transition-all">
                    <span><i class="fa-solid fa-clipboard-list"></i></span> <?php echo e(__('app.portal.submit_exception')); ?>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(! $hasActivePackage): ?>
            <div class="animate-fade-in-up p-6 bg-rose-50/90 border border-rose-200 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-5 text-rose-950 shadow-sm hover:shadow-md transition-all">
                ...
                <a href="<?php echo e(route('courses')); ?>" class="btn-lift px-6 py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl font-bold text-xs shadow-md shadow-rose-600/30 whitespace-nowrap flex items-center gap-2 shrink-0">
                    <span><i class="fa-solid fa-cart-shopping"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'تصفح الكورسات والباقات الآن' : 'Browse Courses & Packages'); ?>
        ...
        <div id="packages" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10 md:mb-12 scroll-mt-28">
            ...
            <div class="animate-fade-in-up stagger-1 glass-card rounded-3xl p-6 sm:p-7 border border-slate-200/80 border-t-4 <?php echo e($hasActivePackage ? 'border-t-teal-500' : 'border-t-rose-500'); ?> shadow-sm hover:shadow-xl transition-all space-y-3.5">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-11 h-11 rounded-2xl <?php echo e($hasActivePackage ? 'bg-teal-50 text-teal-600 border border-teal-100' : 'bg-rose-50 text-rose-600 border border-rose-100'); ?> flex items-center justify-center text-xl shadow-2xs"><i class="fa-solid fa-credit-card"></i></div>
                </div>
            ...
            <div class="lg:col-span-8 space-y-8 lg:space-y-10">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                        <div>
                            <h2 class="font-heading font-black text-xl sm:text-2xl text-slate-900 flex items-center gap-2">
                                <span><i class="fa-solid fa-calendar-days"></i></span> <?php echo e(__('app.portal.upcoming_sessions')); ?>
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(! $hasActivePackage): ?>
                            <div class="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-950 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
                                ...
                                <a href="<?php echo e(route('courses')); ?>" class="btn-lift px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-bold font-mono shadow-sm flex items-center gap-1.5 whitespace-nowrap self-stretch sm:self-auto justify-center">
                                    <span><i class="fa-solid fa-cart-shopping"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'عرض الكورسات والتفعيل' : 'Explore Courses & Activate'); ?>
                            ...
                            <div class="p-5 bg-slate-50/80 hover:bg-slate-50 rounded-2xl border border-slate-200/90 space-y-4 transition-all hover:shadow-md hover:-translate-y-0.5">
                                <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2 text-xs font-mono font-bold">
                                        <span class="bg-blue-100 text-blue-900 px-3 py-1 rounded-full border border-blue-200 whitespace-nowrap">
                                            <i class="fa-solid fa-calendar-days"></i> <?php echo e(app()->getLocale() === 'ar' ? 'البداية' : 'Start'); ?>: <?php echo e($startAt ? $startAt->format('Y-m-d h:i A') : 'Scheduled'); ?>
                                        ...
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($endAt): ?>
                                            <span class="bg-slate-200/80 text-slate-800 px-3 py-1 rounded-full border border-slate-300/60 whitespace-nowrap">
                                                <i class="fa-solid fa-stopwatch"></i> <?php echo e(app()->getLocale() === 'ar' ? 'النهاية' : 'End'); ?>: <?php echo e($endAt->format('h:i A')); ?>
                                        ...
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($halfAt): ?>
                                            <span class="bg-amber-100/90 text-amber-900 px-3 py-1 rounded-full border border-amber-300/80 whitespace-nowrap" title="<?php echo e(app()->getLocale() === 'ar' ? 'آخر موعد للدخول هو منتصف وقت الحصة' : 'Last allowed join time is half-session'); ?>">
                                                <i class="fa-solid fa-hourglass-half"></i> <?php echo e(app()->getLocale() === 'ar' ? 'إغلاق الدخول' : 'Cutoff'); ?>: <?php echo e($halfAt->format('h:i A')); ?>
                                ...
                                <div class="flex flex-wrap items-center justify-between gap-4 pt-3 border-t border-slate-200/70 text-xs font-mono text-slate-700">
                                    <div class="flex flex-wrap items-center gap-4">
                                        <span><i class="fa-solid fa-chalkboard-user"></i> <?php echo e(app()->getLocale() === 'ar' ? 'المدرس' : 'Instructor'); ?>: <strong><?php echo e($s->teacherProfile?->user?->name ?: 'Dr. Instructor'); ?></strong></span>
                                        <span><i class="fa-solid fa-book-open"></i> <?php echo e(app()->getLocale() === 'ar' ? 'المادة' : 'Subject'); ?>: <strong><?php echo e($s->subject?->name ?: 'Physics'); ?></strong></span>
                                    </div>
                                    ...
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($state === \App\Enums\LiveSessionState::LIVE): ?>
                                        <a href="<?php echo e(route('student.meeting.show', ['id' => $s->id])); ?>" class="btn-lift px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-xs shadow-md shadow-emerald-600/30 flex items-center gap-2">
                                            <span><i class="fa-solid fa-circle text-emerald-500 text-[10px]"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'انضم للبث المباشر الان' : 'Join Live Stream'); ?>
                                    ...
                                    <?php elseif($state === \App\Enums\LiveSessionState::PACKAGE_REQUIRED): ?>
                                        <a href="<?php echo e(route('courses')); ?>" class="btn-lift text-xs font-mono font-extrabold bg-rose-50 hover:bg-rose-100 text-rose-800 border border-rose-200/90 px-4 py-2 rounded-xl flex items-center gap-1.5 transition-all shadow-2xs">
                                            <span><i class="fa-solid fa-lock"></i></span> <?php echo e($state->label()); ?>
                                    ...
                                    <?php elseif($state === \App\Enums\LiveSessionState::ENDED): ?>
                                        <span class="text-xs font-mono font-bold bg-slate-100 text-slate-600 border border-slate-300 px-4 py-2 rounded-xl flex items-center gap-1.5">
                                            <span><i class="fa-solid fa-stop"></i>️</span> <?php echo e($state->label()); ?>
                                    ...
                                    <?php elseif($state === \App\Enums\LiveSessionState::PREREQUISITE_REQUIRED): ?>
                                        <span class="text-xs font-mono font-bold bg-amber-100 text-amber-900 border border-amber-300 px-4 py-2 rounded-xl flex items-center gap-1.5">
                                            <span><i class="fa-solid fa-triangle-exclamation"></i></span> <?php echo e($state->label()); ?>
                            ...
                            <div class="py-10 text-center text-slate-500 space-y-3 bg-slate-50/50 rounded-2xl border border-slate-200/80 p-6">
                                ...
                                <div class="pt-2">
                                    <a href="<?php echo e(route('courses')); ?>" class="btn-lift inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold font-mono rounded-xl shadow-md shadow-indigo-600/20">
                                        <span><i class="fa-solid fa-rocket"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'استكشاف الكورسات والباقات المتاحة' : 'Explore Available Courses & Packages'); ?>
                ...
                <div id="assignments" class="glass-card rounded-3xl p-6 sm:p-8 md:p-9 border border-slate-200/80 shadow-sm hover:shadow-lg transition-all space-y-6 animate-fade-in-up stagger-2 scroll-mt-28">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                        <div>
                            <h2 class="font-heading font-black text-xl sm:text-2xl text-slate-900 flex items-center gap-2">
                                <span><i class="fa-solid fa-pen-to-square"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'قسم الواجبات والاختبارات التفاعلية (Assignments & Quizzes)' : 'Assignments & MSQ Quizzes Department'); ?>
                            ...
                            <div class="available-assign-card p-6 bg-gradient-to-br from-teal-50/70 via-emerald-50/30 to-white rounded-3xl border border-teal-200/80 space-y-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all" data-course-id="<?php echo e($assign->course_id ?? 0); ?>">
                                ...
                                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-teal-100/80 pb-3 text-xs font-mono">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="font-bold text-teal-900 bg-teal-100/90 px-3 py-0.5 rounded-full border border-teal-200">
                                            <i class="fa-solid fa-book-open"></i> <?php echo e($courseTitle); ?>

                                        </span>
                                        <span class="font-bold text-slate-800 bg-slate-200/80 px-3 py-0.5 rounded-full">
                                            <i class="fa-solid fa-tv"></i> <?php echo e($sessionTitle); ?>
                                        ...
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isInProgress): ?>
                                            <span class="bg-amber-500 text-white px-2.5 py-0.5 rounded-full font-bold text-[10px] animate-pulse">
                                                <i class="fa-solid fa-bolt"></i> <?php echo e(app()->getLocale() === 'ar' ? 'قيد الحل حالياً' : 'In Progress'); ?>

                                            </span>
                                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    </div>
                                    <span class="text-teal-700 font-bold bg-teal-100/60 px-2.5 py-0.5 rounded-md">
                                        <i class="fa-solid fa-stopwatch"></i> <?php echo e($assign->duration_minutes ?: 30); ?> <?php echo e(app()->getLocale() === 'ar' ? 'دقيقة إجابة' : 'Mins Duration'); ?>
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    ...
                                    <span class="text-xs font-mono font-extrabold text-slate-800 bg-white px-3.5 py-1.5 rounded-xl border border-slate-200 shadow-2xs self-start sm:self-auto">
                                        <i class="fa-solid fa-bullseye"></i> <?php echo e(app()->getLocale() === 'ar' ? 'درجة النجاح:' : 'Pass Mark:'); ?> <?php echo e(number_format($assign->passing_score ?? 70, 0)); ?>%
                                    </span>
                                </div>
                                ...
                                <div class="flex flex-wrap items-center justify-between gap-3 pt-2 text-xs font-mono">
                                    <div class="flex flex-wrap items-center gap-3 text-slate-600">
                                        <span class="flex items-center gap-1 font-bold text-amber-900 bg-amber-50 px-3 py-1 rounded-full border border-amber-200">
                                            <i class="fa-solid fa-clock"></i> <?php echo e(app()->getLocale() === 'ar' ? 'الموعد النهائي' : 'Deadline'); ?>: 
                                        ...
                                        <span class="bg-slate-100 text-slate-800 px-3 py-1 rounded-full border border-slate-200 font-bold">
                                            <i class="fa-solid fa-lock"></i> <?php echo e(app()->getLocale() === 'ar' ? 'محاولة واحدة فقط' : '1 Attempt Only'); ?>
                                    ...
                                    <div class="flex items-center gap-2">
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isInProgress): ?>
                                            <a href="<?php echo e(route('student.assignment.take', ['id' => $assign->id])); ?>" class="btn-lift px-6 py-3 bg-amber-500 hover:bg-amber-600 text-slate-950 rounded-xl font-extrabold text-xs shadow-md shadow-amber-500/30 flex items-center gap-2">
                                                <span><i class="fa-solid fa-bolt"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'استكمال حل الواجب' : 'Resume Assignment'); ?> &rarr;
                                            </a>
                                        <?php else: ?>
                                            <a href="<?php echo e(route('student.assignment.take', ['id' => $assign->id])); ?>" class="btn-lift px-6 py-3 bg-[#0D9488] hover:bg-[#0F766E] text-white rounded-xl font-extrabold text-xs shadow-md shadow-teal-600/30 flex items-center gap-2">
                                                <span><i class="fa-solid fa-bolt"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'بدء حل الواجب التفاعلي' : 'Start Interactive MSQ'); ?>
                                        ...
                                        <button onclick="openMsqAssignmentModal(<?php echo e($assign->id); ?>)" class="btn-lift px-4 py-3 bg-white hover:bg-slate-100 text-slate-800 rounded-xl font-bold text-xs border border-slate-300 shadow-2xs cursor-pointer">
                                            <?php echo e(app()->getLocale() === 'ar' ? 'معاينة سريعة <i class="fa-solid fa-eye"></i>' : 'Quick Preview <i class="fa-solid fa-eye"></i>'); ?>
                ...
                <div class="glass-card rounded-3xl p-6 sm:p-8 md:p-9 border border-slate-200/80 shadow-sm hover:shadow-lg transition-all space-y-6 animate-fade-in-up stagger-3">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                        <div>
                            <h2 class="font-heading font-black text-xl sm:text-2xl text-slate-900 flex items-center gap-2">
                                <span><i class="fa-solid fa-book-open"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'الكورسات المشترك بها والمنهج التفصيلي' : 'My Enrolled Courses & Detailed Modules'); ?>
                                ...
                                <div class="bg-white rounded-3xl p-6 border border-slate-200/90 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group space-y-4">
                                    <div class="space-y-3">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span class="text-[11px] font-mono font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200">
                                                <i class="fa-solid fa-check"></i> <?php echo e(app()->getLocale() === 'ar' ? 'مشترك بنجاح' : 'Enrolled'); ?>
                                        ...
                                        <div class="flex items-center gap-2 pt-1 text-xs font-mono text-slate-700">
                                            <img src="<?php echo e(asset('images/instructor_portrait.webp')); ?>" alt="<?php echo e($card['teacher']); ?>" class="w-6 h-6 rounded-full object-cover border border-teal-500">
                                            <span><i class="fa-solid fa-chalkboard-user"></i> <strong><?php echo e($card['teacher']); ?></strong></span>
                                        </div>
                                    ...
                                    <div class="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                                        <div class="flex items-center gap-2 text-xs font-mono text-slate-500">
                                            <span><i class="fa-solid fa-video"></i> <strong><?php echo e($card['recCount']); ?></strong> <?php echo e(app()->getLocale() === 'ar' ? 'دروس' : 'Lectures'); ?></span>
                                            <span>•</span>
                                            <span><i class="fa-solid fa-circle text-emerald-500 text-[10px]"></i> <strong><?php echo e($card['liveCount']); ?></strong> <?php echo e(app()->getLocale() === 'ar' ? 'بث مباشر' : 'Live Streams'); ?></span>
                                        </div>
                                        ...
                                        <div class="flex items-center gap-2">
                                            <button onclick="openEnrolledCourseModal(<?php echo e($card['course']->id); ?>)" class="btn-lift px-4 py-2 bg-slate-900 hover:bg-teal-600 text-white rounded-xl text-xs font-extrabold shadow-md flex items-center gap-1.5 cursor-pointer transition-all">
                                                <span><i class="fa-solid fa-magnifying-glass"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'التفاصيل والحصص' : 'Full Details & Sessions'); ?>
                        ...
                        <div class="py-10 text-center text-slate-500 space-y-3 bg-slate-50/50 rounded-2xl border border-slate-200/80 p-6">
                            ...
                            <div class="pt-2">
                                <a href="<?php echo e(route('courses')); ?>" class="btn-lift inline-flex items-center gap-2 px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold font-mono rounded-xl shadow-md shadow-teal-600/20">
                                    <span><i class="fa-solid fa-rocket"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'تصفح الكورسات المتاحة الآن' : 'Browse Available Courses'); ?>
                ...
                <div class="glass-card rounded-3xl p-6 sm:p-8 md:p-9 border border-slate-200/80 shadow-sm hover:shadow-lg transition-all space-y-6 animate-fade-in-up stagger-3">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                        <div>
                            <h2 class="font-heading font-black text-xl sm:text-2xl text-slate-900 flex items-center gap-2">
                                <span><i class="fa-solid fa-scroll"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'سجل تسليمات الواجبات والدرجات (Submissions History)' : 'Assignment Submission History & Graded Evaluation'); ?>
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $submissions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                            ...
                            <div class="submission-record-card p-6 bg-slate-50/90 hover:bg-slate-100/80 rounded-3xl border border-slate-200/90 space-y-4 transition-all hover:shadow-md hover:-translate-y-0.5" data-course-id="<?php echo e($subCourseId); ?>">
                                ...
                                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200/70 pb-3 text-xs font-mono">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="font-bold text-teal-900 bg-teal-100/90 px-3 py-0.5 rounded-full border border-teal-200">
                                            <i class="fa-solid fa-book-open"></i> <?php echo e($subCourseTitle); ?>

                                        </span>
                                        <span class="font-bold text-slate-800 bg-slate-200 px-3 py-0.5 rounded-full">
                                            <i class="fa-solid fa-tv"></i> <?php echo e($subSessionTitle); ?>

                                        </span>
                                    </div>
                                    <span class="text-slate-500">
                                        <i class="fa-solid fa-calendar-days"></i> <?php echo e(app()->getLocale() === 'ar' ? 'تاريخ التسليم' : 'Submitted'); ?>: <strong><?php echo e($sub->submitted_at ? $sub->submitted_at->format('Y-m-d H:i') : 'Completed'); ?></strong>
                                    </span>
                                </div>
                                    ...
                                    <div class="flex items-center gap-3 self-start sm:self-auto">
                                        ...
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($sub->isPassed()): ?>
                                            <span class="text-xs font-mono font-extrabold bg-emerald-100 text-emerald-900 px-4 py-2 rounded-2xl border border-emerald-300 flex items-center gap-1.5 shadow-2xs">
                                                <i class="fa-solid fa-check"></i> <?php echo e(app()->getLocale() === 'ar' ? 'ناجح' : 'PASSED'); ?>
                                        ...
                                        <?php else: ?>
                                            <span class="text-xs font-mono font-extrabold bg-rose-100 text-rose-900 px-4 py-2 rounded-2xl border border-rose-300 flex items-center gap-1.5 shadow-2xs">
                                                <i class="fa-solid fa-xmark"></i> <?php echo e(app()->getLocale() === 'ar' ? 'لم يجتاز' : 'FAILED'); ?>
            ...
            <div class="lg:col-span-4 space-y-8">
                ...
                <div class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-xl space-y-5">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                        <h3 class="font-heading font-black text-xl text-slate-900 flex items-center gap-2">
                            <span><i class="fa-solid fa-bell"></i></span> <?php echo e(__('app.portal.notifications')); ?>
                ...
                <div id="teacher-notes" class="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm hover:shadow-lg transition-all space-y-5 animate-fade-in-up stagger-2 scroll-mt-28">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <h3 class="font-heading font-black text-xl text-slate-900 flex items-center gap-2">
                            <span><i class="fa-solid fa-comments"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'ملاحظات المعلمين والتوجيه الأكاديمي' : 'Teacher Notes & Pedagogical Feedback'); ?>
                        ...
                        <span class="px-2.5 py-1 bg-teal-50 text-teal-800 border border-teal-200/80 text-[11px] font-mono font-extrabold rounded-xl shrink-0 whitespace-nowrap shadow-2xs">
                            <?php echo e(count($teacherNotes)); ?> <?php echo e(app()->getLocale() === 'ar' ? 'ملاحظات' : 'Notes'); ?>

                        </span>
                    </div>

                    <div class="space-y-3.5">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $teacherNotes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $n): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                            <?php
                                ...
                                <div class="flex items-center justify-between gap-2 flex-wrap">
                                    <div class="flex items-center gap-2.5 min-w-0">
                                        <div class="w-8 h-8 rounded-xl bg-gradient-to-tr from-teal-500 to-emerald-400 text-slate-950 font-heading font-black text-xs flex items-center justify-center shrink-0 shadow-2xs">
                                            <?php echo e(mb_substr($n->teacherProfile?->user?->name ?? 'T', 0, 1)); ?>

                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-xs font-bold text-slate-900 truncate"><?php echo e($n->teacherProfile?->user?->name ?: __('Academic Teacher')); ?></p>
                                            <p class="text-[10px] font-mono text-slate-500 truncate"><?php echo e($n->teacherProfile?->subjects?->pluck('name')->join(', ') ?: 'Elite Academy Faculty'); ?></p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-2 shrink-0">
                                        <span class="px-2.5 py-0.5 text-[10px] font-mono font-bold rounded-full border <?php echo e($catConfig['bg']); ?>">
                                            <?php echo e($catConfig['label']); ?>

                                        </span>
                                        <span class="text-[10px] font-mono text-slate-400"><?php echo e($n->created_at ? $n->created_at->diffForHumans() : ''); ?></span>
                                    </div>
                                </div>
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                            <div class="p-6 bg-slate-50 rounded-2xl border border-slate-200 text-center space-y-2">
                                ...
                                <p class="text-xs font-semibold text-slate-700"><?php echo e(app()->getLocale() === 'ar' ? 'لا توجد ملاحظات مسجلة لك من المعلمين حتى الآن.' : 'No pedagogical notes recorded by your teachers yet.'); ?></p>
                                <p class="text-[10px] font-mono text-slate-400"><?php echo e(app()->getLocale() === 'ar' ? 'ستظهر هنا أي ملاحظات أو توجيهات أكاديمية يرسلها معلموك.' : 'Any feedback or guidance from your instructors will appear here.'); ?></p>
                            </div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </div>
                </div>
                ...
                <div id="exceptions" class="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm hover:shadow-lg transition-all space-y-5 animate-fade-in-up stagger-2 scroll-mt-28">
                    <h3 class="font-heading font-black text-xl text-slate-900 flex items-center gap-2 border-b border-slate-100 pb-3">
                        <span><i class="fa-solid fa-clipboard-list"></i></span> <?php echo e(__('app.portal.exceptions_history')); ?>
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl space-y-4 border border-slate-200">
        ...
        <form id="excuseForm" action="<?php echo e(route('ajax.exception.submit')); ?>" method="POST" class="space-y-4">
            ...
            <div class="space-y-1.5">
                ...
                <p class="text-[11px] text-amber-700 font-bold"><i class="fa-solid fa-triangle-exclamation"></i> <?php echo e(__('app.sessions.excuse_2h_rule')); ?></p>
            </div>
        ...
        <div class="bg-gradient-to-r from-slate-900 via-slate-950 to-teal-950 text-white p-6 sm:p-8 flex items-start justify-between relative overflow-hidden shrink-0">
            ...
            <div class="space-y-2 relative z-10">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="bg-emerald-500/20 text-emerald-300 text-xs font-mono font-bold px-2.5 py-0.5 rounded-full border border-emerald-500/40"><i class="fa-solid fa-check"></i> <?php echo e(app()->getLocale() === 'ar' ? 'مشترك بالنظام' : 'Enrolled'); ?></span>
                </div>
        ...
        <div class="p-6 sm:p-8 space-y-6 overflow-y-auto max-h-[calc(90vh-180px)] font-mono text-slate-800">
            ...
            <div class="flex items-center justify-between border-b border-slate-200 pb-3">
                <h3 class="font-heading font-black text-lg text-slate-900 flex items-center gap-2">
                    <span><i class="fa-solid fa-tv"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'منهج الكورس والحصص التفصيلية' : 'Full Curriculum & Session Modules'); ?>
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    <h4 class="font-bold text-xs uppercase tracking-wider text-teal-800 flex items-center gap-1.5">
                        <span><i class="fa-solid fa-circle text-emerald-500 text-[10px]"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'جدول الحصص والبث المباشر (Live Streams)' : 'Live Stream Schedule'); ?>
            ...
            <div class="space-y-3 pt-4 border-t border-slate-100">
                <div class="flex items-center justify-between">
                    <h4 class="font-bold text-xs uppercase tracking-wider text-indigo-800 flex items-center gap-1.5">
                        <span><i class="fa-solid fa-video"></i></span> <?php echo e(app()->getLocale() === 'ar' ? 'دروس الفيديو والواجبات المنهجية (Recorded Modules & MSQs)' : 'Recorded Modules & Assignments'); ?>
...
<?php echo $__env->make('layouts.portal-panel', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\elite-academy\resources\views/pages/student-portal.blade.php ENDPATH**/ ?>