        ...
        return $schema
            ->components([
                Tabs::make('SettingsTabs')
                    ->tabs([
                        // ── Tab 1: Currency & Pricing ──
                        Tabs\Tab::make('CurrencySettings')
                            ->label($isAr ? 'إعدادات العملة والأسعار' : 'Currency & Pricing')
                            ->icon('heroicon-o-banknotes')
                            ->schema([
                                Section::make(new HtmlString('<i class="fa-solid fa-coins text-amber-500 me-2"></i>' . ($isAr ? 'العملة الرسمية للمنصة' : 'Platform Currency Configuration')))
                                    ->description($isAr
                                        ? 'تحديد العملة الافتراضية للتعامل المالي وطريقة عرضها للطلاب وأولياء الأمور والمعلمين عبر كافة المقررات والباقات والفواتير.'
                                        : 'Configure active currency code and display symbols across all courses, packages, student portals, and invoices.')
                                    ->schema([
                                        Grid::make(3)->schema([
                                            Select::make('currency_code')
                                                ->label($isAr ? 'كود العملة الدولي (ISO Code)' : 'Currency Code (ISO)')
                                                ->options([
                                            ...
                                            TextInput::make('currency_symbol_ar')
                                                ->label($isAr ? 'رمز العملة (بالعربية)' : 'Currency Symbol (Arabic)')
                                                ->default('ج.م')
                                                ->placeholder('ج.م')
                                                ->required(),

                                            TextInput::make('currency_symbol_en')
                                                ->label($isAr ? 'رمز العملة (بالإنجليزية)' : 'Currency Symbol (English)')
                                                ->default('EGP')
                                                ->placeholder('EGP')
                                                ->required(),
                                        ]),
                                        ...
                                        Grid::make(2)->schema([
                                            Select::make('currency_position')
                                                ->label($isAr ? 'موضع الرمز بالنسبة للرقم' : 'Currency Symbol Position')
                                                ->options([
                                                    'after' => $isAr ? 'بعد المبلغ (مثال: 290 ج.م / 290 EGP)' : 'After Amount (e.g. 290 EGP / 290 ج.م)',
                                                    'before' => $isAr ? 'قبل المبلغ (مثال: ج.م 290 / $ 290)' : 'Before Amount (e.g. EGP 290 / $ 290)',
                                                ])
                                                ->default('after')
                                                ->required()
                                                ->native(false),
                                        ...
                                        Html::make(new HtmlString(
                                            '<div style="background-color: rgba(20, 184, 166, 0.08); border: 1.5px solid rgba(20, 184, 166, 0.35); border-radius: 0.875rem; padding: 1.15rem 1.25rem;">
                                                <div style="display: flex; align-items: center; gap: 0.5rem; font-weight: 800; font-size: 0.875rem; color: #0d9488; margin-bottom: 0.75rem;">
                                                    <i class="fa-solid fa-wand-magic-sparkles text-teal-500"></i>
                                                    <span>' . ($isAr ? 'معاينة فورية لشكل الأسعار:' : 'Live Price Display Preview:') . '</span>
                                                </div>
                                                <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 1.25rem;">
                                                    <div style="display: flex; align-items: center; gap: 0.65rem;">
                                                        <span style="font-size: 0.95rem; font-weight: 700;">' . ($isAr ? 'باللغة العربية:' : 'In Arabic:') . '</span>
                                                        <span style="display: inline-flex; align-items: center; font-family: ui-monospace, monospace; font-size: 1.05rem; font-weight: 900; color: #0f766e; background: rgba(20, 184, 166, 0.14); border: 1px solid rgba(20, 184, 166, 0.25); padding: 0.25rem 0.65rem; border-radius: 0.5rem;">' . format_currency(290, 'ar') . '</span>
                                                    </div>
                                                    <span style="color: #cbd5e1; font-weight: 400; font-size: 1.25rem;">|</span>
                                                    <div style="display: flex; align-items: center; gap: 0.65rem;">
                                                        <span style="font-size: 0.95rem; font-weight: 700;">' . ($isAr ? 'باللغة الإنجليزية:' : 'In English:') . '</span>
                                                        <span style="display: inline-flex; align-items: center; font-family: ui-monospace, monospace; font-size: 1.05rem; font-weight: 900; color: #0f766e; background: rgba(20, 184, 166, 0.14); border: 1px solid rgba(20, 184, 166, 0.25); padding: 0.25rem 0.65rem; border-radius: 0.5rem;">' . format_currency(290, 'en') . '</span>
                                                    </div>
                                                </div>
                                            </div>'
                                        )),
    ...
    public function save(): void
    {
        ...
        Notification::make()
            ...
            ->body(app()->getLocale() === 'ar'
                ? 'تم تحديث روابط السوشيال ميديا وعناصر الفوتر وإعدادات العملة بنجاح.'
                : 'Footer social links, navigation menus and currency settings updated successfully.')
            ->success()
            ->send();
    }
}
