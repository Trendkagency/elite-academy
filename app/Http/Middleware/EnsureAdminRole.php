...
class EnsureAdminRole
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (! $user->isAdmin()) {
            if ($request->expectsJson()) {
                return response()->json(['success' => false, 'message' => __('Unauthorized Access. Admin privilege required.')], 403);
            }

            abort(403, __('Unauthorized Access. Admin area is restricted to system administrators.'));
        }
