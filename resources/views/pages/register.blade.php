    ...
    <div class="max-w-md mx-auto bg-white rounded-3xl p-8 sm:p-10 border border-slate-200/90 shadow-xl space-y-6">
        <div class="text-center space-y-2">
            ...
            <p class="text-xs text-slate-500 font-mono">
                {{ __('Join Elite Academy and start interactive learning.') }}
            </p>
        </div>
        ...
        <form id="registerForm" action="{{ route('ajax.register') }}" method="POST" class="space-y-4">
            ...
            <div class="space-y-1.5">
                <label class="text-xs font-bold text-slate-700">{{ __('Full Name') }}</label>
                <input type="text" name="name" required placeholder="{{ __('e.g. David Kovacs') }}" class="input-mobile">
            </div>

            <div class="space-y-1.5">
                <label class="text-xs font-bold text-slate-700">{{ __('Email Address') }}</label>
            ...
            <div class="space-y-1.5">
                <label class="text-xs font-bold text-slate-700 flex items-center justify-between">
                    ...
                    <span class="text-[10px] text-rose-500 font-bold">* {{ __('Required') }}</span>
                </label>
            ...
            <div id="studentFieldsGroup" class="space-y-4 pt-2">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-bold text-slate-700 flex items-center justify-between">
                        ...
                        <span class="text-[10px] text-teal-600 font-bold">* {{ __('Required for Student') }}</span>
                    </label>
                ...
                {{-- School Name --}}
                <div class="space-y-1.5">
                    <label class="text-xs font-bold text-slate-700">{{ __('School Name (Optional)') }}</label>
                    <input type="text" name="school_name" placeholder="{{ __('e.g. Cairo International School') }}" class="input-mobile">
                </div>
            </div>

            <div class="space-y-1.5">
                <label class="text-xs font-bold text-slate-700">{{ __('Password') }}</label>
            ...
            <button type="submit" id="submitBtn" class="w-full btn-mobile-lg btn-lift text-white bg-teal-600 hover:bg-teal-700 shadow-md shadow-teal-600/20 touch-press mt-2 flex items-center justify-center gap-2 font-bold py-3.5 rounded-2xl">
                <span>{{ __('Create Account & Start') }}</span>
