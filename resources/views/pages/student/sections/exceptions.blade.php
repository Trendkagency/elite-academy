                ...
                <table class="w-full text-start text-xs elite-sortable-table" data-page-size="6">
                    <thead class="bg-slate-50 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 font-mono font-bold uppercase text-[11px] border-b border-slate-200 dark:border-slate-700 select-none">
                        <tr>
                            ...
                            <th class="py-3.5 px-4 text-end" data-no-sort="true">{{ $isAr ? 'حالة الاعتماد' : 'Approval Status' }}</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                                <td class="py-3.5 px-4 font-bold text-slate-900 dark:text-white whitespace-nowrap">
                                    <div class="space-y-0.5">
                                        <span>{{ $exc->is_global || $exc->scope === 'global' ? ($isAr ? 'استثناء شامل (كافة الكورسات)' : 'Global System Exemption') : ($isAr ? 'عذر كورس محدد' : 'Course Exception') }}</span>
                                        <p class="text-[10px] text-slate-400 font-normal">#EXC-{{ $exc->id }}</p>
                                    </div>
                                </td>

                                <td class="py-3.5 px-3 whitespace-nowrap text-slate-700 dark:text-slate-300">
                                    {{ $exc->liveSession?->title ?: ($exc->course?->title ?: ($isAr ? 'طلب عام' : 'General Request')) }}
                                </td>

                                <td class="py-3.5 px-3 allow-wrap text-slate-600 dark:text-slate-400 max-w-[280px]">
                                    {{ $exc->reason }}
                                </td>
                                ...
                                <td class="py-3.5 px-4 text-end whitespace-nowrap">
                                    @if($exc->status === 'approved')
                                        <span class="px-3 py-1 rounded-full text-xs font-extrabold bg-emerald-100 text-emerald-900 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 inline-flex items-center gap-1">
                                            <i class="fa-solid fa-check"></i> {{ $isAr ? 'تم القبول والاعتماد' : 'Approved' }}
                                        </span>
                                    @elseif($exc->status === 'rejected')
                                        <span class="px-3 py-1 rounded-full text-xs font-extrabold bg-rose-100 text-rose-900 dark:bg-rose-950 dark:text-rose-300 border border-rose-300 dark:border-rose-800 inline-flex items-center gap-1">
                                            <i class="fa-solid fa-xmark"></i> {{ $isAr ? 'مرفوض' : 'Rejected' }}
                                        </span>
                                    @else
                                        <span class="px-3 py-1 rounded-full text-xs font-extrabold bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-300 border border-amber-300 dark:border-amber-800 inline-flex items-center gap-1">
                                            <i class="fa-solid fa-clock"></i> {{ $isAr ? 'قيد المراجعة' : 'Pending Review' }}
                                        </span>
                                    @endif
                                </td>
                            </tr>
