                ...
                [
                    ...
                    'mainEntity' => [
                        [
                            ...
                            'name' =>
                                app()->getLocale() === 'ar'
                                    ? 'هل تشمل هذه الدورة حصصاً تفاعلية مباشرة وتسجيلات؟'
                                    : 'Does this course include live interactive sessions and recordings?',
                            'acceptedAnswer' => [
                                '@type' => 'Answer',
                                'text' =>
                                    app()->getLocale() === 'ar'
                        ...
                        [
                            ...
                            'name' =>
                                app()->getLocale() === 'ar'
                                    ? 'هل يتم تقديم شهادة إتمام معتمدة؟'
                                    : 'Is an accredited certificate issued upon completion?',
                            'acceptedAnswer' => [
                                '@type' => 'Answer',
                                'text' =>
                                    app()->getLocale() === 'ar'
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="space-y-3 max-w-3xl">
                <div class="flex items-center gap-3">
                    ...
                    @if ($isEnrolled ?? false)
                        <span class="bg-teal-500 text-white text-xs font-bold px-3 py-1 rounded-full"><i
                                class="fa-solid fa-check"></i> Enrolled Course</span>
                    @else
                        <span class="bg-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full">▶ Free Demo
                            Available</span>
                    @endif
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
                <div class="lg:col-span-8 space-y-12">
                    ...
                    @php
                        ...
                        $demoTitle =
                            $isArabicTitle || app()->getLocale() === 'ar'
                        ...
                        $videoData = $course
                            ? $course->getVideoEmbedData()
                            : ['type' => 'mp4', 'embed_url' => asset('videos/physics_demo.mp4')];
                        $posterImage =
                            $course && $course->image
                                ? media_url($course->image, 'images/course_ai.webp')
                                : asset('images/course_ai.webp');
                    @endphp
                    <div id="demo"
                        class="bg-gradient-to-br from-teal-900 via-slate-900 to-teal-950 text-white rounded-3xl p-8 border border-teal-500/40 shadow-xl space-y-6">
                        <div class="flex items-center justify-between border-b border-teal-500/30 pb-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="font-mono text-xs font-bold uppercase tracking-widest text-orange-400">{{ __('Interactive Preview') }}</span>
                            </div>
                            <span
                                class="text-xs font-mono bg-teal-800/80 text-teal-200 px-3 py-1 rounded-full border border-teal-500/30">{{ __('Free Demo Lesson') }}</span>
                        </div>
                    ...
                    @include('components.curriculum-timeline', [
                        ...
                        'subtitle' =>
                            'Structured timeline of lectures, live coding labs, and homework assignments',
                    ])
                </div>
                ...
                <div class="lg:col-span-4 space-y-6">
                    @php
                        $nextLiveSession = $course?->liveSessions
                            ?->where('scheduled_at', '>=', now())
                            ->sortBy('scheduled_at')
                            ->first();
                        $targetDate = $nextLiveSession
                            ? $nextLiveSession->scheduled_at->toIso8601String()
                            : now()->addDays(3)->setTime(18, 0)->toIso8601String();
                    ...
                    @include('components.course-countdown-timer', [
                        ...
                        'title' =>
                            app()->getLocale() === 'ar' ? 'عداد البث المباشر القادم' : 'Live Cohort Start Timer',
                        'subtitle' =>
                            app()->getLocale() === 'ar'
                                ? 'الوقت المتبقي لإنطلاق حصة البث المباشر التفاعلية'
                                : 'Countdown to upcoming interactive live stream',
                    ])
                    ...
                    <div class="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-2xs space-y-6">
                        ...
                        @auth
                            @if ($isEnrolled ?? false)
                                <a href="{{ route('student-portal') }}"
                                    class="btn-lift w-full inline-flex items-center justify-center gap-2 text-center py-3.5 px-6 font-semibold text-white bg-teal-600 hover:bg-teal-700 rounded-xl shadow-md transition-all">
                                    <span>{{ app()->getLocale() === 'ar' ? 'مشترك في هذا الكورس' : 'Enrolled Course' }}</span>
                                    <i class="fa-solid fa-check"></i>
                                    <span>{{ app()->getLocale() === 'ar' ? '— الذهاب لبوابة الطالب ←' : '— Go to Student Portal &rarr;' }}</span>
                                </a>
                            @else
                                <button id="btnEnroll"
                                    class="w-full inline-flex items-center justify-center gap-2 text-center py-3.5 px-6 font-semibold text-white bg-orange-500 hover:bg-orange-600 rounded-xl shadow-md transition-all cursor-pointer">
                                    <span>{{ app()->getLocale() === 'ar' ? 'التسجيل في الكورس الآن' : 'Enroll in Course Now' }}</span>
                                    <i class="fa-solid fa-rocket"></i>
                                </button>
                            @endif
                        @else
                            <a href="{{ route('login') }}"
                                class="w-full inline-block text-center py-3.5 px-6 font-semibold text-white bg-teal-600 hover:bg-teal-700 rounded-xl shadow-md transition-all">
                        ...
                        <div class="pt-4 border-t border-slate-100 space-y-3">
                            ...
                            <div class="flex items-center gap-3">
                                <img src="{{ asset('images/instructor_portrait.webp') }}" alt="{{ $cTeacher }}"
                                    class="w-10 h-10 rounded-xl object-cover border border-teal-500">
                                <div>
                                    <a href="{{ route('teachers') }}"
                                        class="text-xs font-bold text-slate-900 hover:text-teal-600">{{ $cTeacher }}</a>
        ...
        @if (!($isEnrolled ?? false))
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const defaultBtnHtml = `<span>{{ app()->getLocale() === 'ar' ? 'التسجيل في الكورس الآن' : 'Enroll in Course Now' }}</span> <i class="fa-solid fa-rocket"></i>`;

                    btn.addEventListener('click', async function() {
                        btn.disabled = true;
                        btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> <span>{{ app()->getLocale() === 'ar' ? 'جاري التسجيل...' : 'Enrolling...' }}</span>`;
                        ...
                        try {
                            ...
                            alertBox.className =
                                `p-3 rounded-xl text-xs font-semibold ${data.success ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200'}`;
                            ...
                            if (data.success) {
                                ...
                                btn.outerHTML =
                                    `<a href="${portalUrl}" class="btn-lift w-full inline-flex items-center justify-center gap-2 text-center py-3.5 px-6 font-semibold text-white bg-teal-600 hover:bg-teal-700 rounded-xl shadow-md transition-all"><span>${linkText}</span> <i class="fa-solid fa-check"></i></a>`;
                            } else {
                                btn.disabled = false;
                            ...
                            btn.innerHTML = defaultBtnHtml;
                            alertBox.className =
                                'p-3 rounded-xl text-xs font-semibold bg-red-50 text-red-700 border border-red-200';
