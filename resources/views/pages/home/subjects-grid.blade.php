    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-5 md:gap-8">
            @foreach ($homeSubjects as $sub)
                <div class="anim-subject delay-3 sr-card aspect-[4/5] md:aspect-auto md:h-[520px] rounded-[24px] bg-{{ $sub['color'] }}-950 text-white shadow-lg shadow-{{ $sub['color'] }}-950/10 hover:shadow-2xl card-lift flex flex-col justify-between overflow-hidden group transition-all duration-300 relative active:scale-[0.98]">
                    <div class="absolute inset-0 md:relative md:h-[338px] overflow-hidden bg-slate-950">
                        <img src="{{ asset($sub['img']) }}" alt="{{ $sub['name'] }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 ease-out">
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/30 to-transparent md:from-{{ $sub['color'] }}-950 md:via-transparent pointer-events-none"></div>
                    </div>

                    <div class="absolute bottom-0 inset-x-0 p-3.5 sm:p-4 text-white z-10 flex flex-col justify-end space-y-1 md:relative md:p-6 md:flex-1 md:bg-{{ $sub['color'] }}-950 md:space-y-3">
                        <div class="space-y-1">
                            <span class="text-[9px] sm:text-[10px] font-mono font-extrabold uppercase tracking-widest text-{{ $sub['color'] }}-300 bg-slate-950/70 md:bg-transparent backdrop-blur-xs md:backdrop-blur-none px-2 py-0.5 md:p-0 rounded-full md:rounded-none border border-white/10 md:border-none inline-block w-max">{{ $sub['tag'] }}</span>
                            <h3 class="font-heading font-extrabold text-sm sm:text-base md:text-2xl text-white group-hover:text-{{ $sub['color'] }}-300 transition-colors line-clamp-2 leading-snug">
                                <a href="{{ route('subject-details') }}">{{ $sub['name'] }}</a>
                            </h3>
                        </div>

                        <div class="hidden md:flex items-center justify-between pt-3 border-t border-{{ $sub['color'] }}-900 text-xs text-{{ $sub['color'] }}-100 font-medium">
                            <span>👨‍🏫 {{ $sub['teachers'] }} Teachers • 📚 {{ $sub['courses'] }} Courses</span>
                            <a href="{{ route('subject-details') }}" class="text-xs font-extrabold text-{{ $sub['color'] }}-300 group-hover:text-{{ $sub['color'] }}-200 flex items-center gap-1">
