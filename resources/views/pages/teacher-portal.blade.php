    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-2xl w-full shadow-2xl border border-slate-200 space-y-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <form id="createAssignmentForm" action="{{ route('ajax.teacher.assignments.create') }}" method="POST" class="space-y-5">
                    ...
                    <select name="live_session_id" class="input-mobile bg-white">
                        ...
                        @if(isset($allSessions))
                            @foreach($allSessions as $s)
                                <option value="{{ $s->id }}">Session #{{ $s->id }} — {{ $s->title ?? $s->course?->title ?? 'Live Session' }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    <label class="block text-xs font-mono font-bold text-slate-500 uppercase tracking-wider mb-1.5">{{ __('Timer (Minutes)') }} *</label>
            ...
            {{-- Interactive Questions Builder Section --}}
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="font-heading font-black text-sm text-slate-900 flex items-center gap-1.5">
                            <span>❓</span> {{ __('Interactive MSQ Questions (Optional)') }}
                        </h4>
                        <p class="text-[11px] font-mono text-slate-500">{{ __('Add multiple-choice questions for instant server-side auto-grading.') }}</p>
                    </div>
                    <button type="button" onclick="addTeacherQuestion()" class="btn-lift px-3.5 py-1.5 bg-teal-50 hover:bg-teal-100 text-teal-800 rounded-xl font-extrabold text-xs border border-teal-200/80 flex items-center gap-1 cursor-pointer">
                        <span>➕</span> {{ __('Add Question') }}
                    </button>
                </div>

                <div id="teacherQuestionsContainer" class="space-y-4">
                    {{-- Dynamically injected question cards --}}
                </div>
            </div>

            <div class="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
                <button type="button" onclick="closeModal('createAssignmentModal')" class="px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-xl cursor-pointer">{{ __('Cancel') }}</button>
