    ...
    <div class="flex-1 overflow-y-auto px-4 py-6 space-y-6 scrollbar-thin scrollbar-thumb-slate-800">
        ...
        <div class="space-y-1">
            ...
            @elseif($role === 'parent')
                ...
                <button type="button" onclick="document.getElementById('linkChildModal').classList.remove('hidden')" class="w-full text-{{ $isAr ? 'right' : 'left' }} flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all group hover:bg-amber-950/60 hover:text-amber-300 text-amber-400 cursor-pointer">
